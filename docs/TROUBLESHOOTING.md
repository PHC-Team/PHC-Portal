...
4. Click OK, then **fully close PHC-Portal from Task Manager** and reopen it.
...
### Step 3 — Add PHC-Portal to Windows Defender exclusions
...
3. Add the entire PHC-Portal folder (e.g. `C:\PHC-Portal\`) as a **Folder** exclusion.

Restart PHC-Portal after adding the exclusion.

---

### Step 4 — Run PHC-Portal as Administrator

Right-click `PHC-Portal_GUI.exe` → **Run as administrator**. Some sandbox/permission configurations prevent Chromium sub-processes from launching without elevated rights.

---

### Step 5 — Diagnose with Task Manager

Open **Task Manager** while PHC-Portal is running and look for a process called `QtWebEngineProcess.exe`:
...
Press `Win + R` → type `eventvwr` → **Windows Logs** → **Application**. Look for crash or error entries mentioning `QtWebEngineProcess` or `PHC-Portal_GUI`. The error description will identify whether it is a missing DLL, access-denied, or GPU driver issue.
...
PHC-Portal handles this automatically, but if you still see the error:

1. **Workshop ACF fix** — The most common cause is orphaned workshop items in `appworkshop_{id}.acf` triggering a failed Workshop update. PHC-Portal patches this file to clear `NeedsDownload` when no workshop content is installed.
...
3. **ACF error state** — PHC-Portal clears stale `UpdateResult` and validation flags in the game ACF so Steam doesn't get stuck in a retry loop.

---

## Steam path not found

PHC-Portal needs to know where Steam is installed. If it cannot find it automatically, it will ask you to choose the folder. Pick the folder that contains `steam.exe` (usually `C:\Program Files (x86)\Steam`).
...
SteamAutoCrack is bundled inside the `_internal/` folder (shipped alongside `PHC-Portal_GUI.exe`) and is found automatically. You do **not** need to download or extract it manually.

If you still see this error:
- Make sure you are using the latest version of PHC-Portal.
- Install PHC-Portal in a short path like `C:\PHC-Portal\` — very long install paths can cause Windows path-length errors (260-character limit).
...
## Antivirus flags PHC-Portal files

Starting with version 5.3.0, PHC-Portal uses the one-dir distribution format. All files are pre-extracted into the `_internal/` folder next to `PHC-Portal_GUI.exe` at install time. Nothing is extracted to `%TEMP%` at runtime, which greatly reduces false positives.

If your antivirus still flags files, add your PHC-Portal folder (e.g. `C:\PHC-Portal\`) to your AV exclusions. The `_internal/` folder contains Python runtime files and bundled tools — none of them are malware.

---

## Chrome or ChromeDriver errors (multiplayer fix)

If the multiplayer fix needs a browser and you get a Chrome or ChromeDriver error, make sure Chrome is installed and up to date. Try closing all Chrome windows and running PHC-Portal again, or run it as administrator.

---

## Login failed (online-fix.me)

Check your username and password on the online-fix.me website. If you can log in there, update your credentials in PHC-Portal under Settings. Some games may no longer be available on the site.

---

## Download timeout or extraction failed

Check your internet connection. Try disabling antivirus temporarily and run PHC-Portal again. Make sure you have 7-Zip or WinRAR installed if PHC-Portal needs to extract archives. If a download keeps failing, you can try downloading the fix manually from online-fix.me and extracting it into the game folder yourself.

---

## Permission denied or access denied

Steam or the game folder may be in a protected location. Try running PHC-Portal as administrator (right-click → "Run as administrator"). Do not run PHC-Portal from a folder that requires admin rights to write to.

---

## Settings export or import error

If exporting or importing settings fails, try exporting without including sensitive data. Make sure the folder you export to is writable. If you get a message about "JSON serializable", try updating PHC-Portal to the latest version.
...
You can delete `api_cache.json` — PHC-Portal will create a new one when needed. Backup retention is set in Settings; you can lower how many backups are kept.
...
**Fixed in 1.0.0:** The log panel now evicts old entries, keeping only the last 1000 lines. RAM usage stays flat regardless of download size.

If you are still seeing high RAM on an older version, update to 1.0.0 or newer.
...
**Fixed in 1.0.0:** The button now routes through `_startDownload`, which shows the library selection dialog whenever more than one Steam library is detected before the download begins.
...
Read the error message first — it often explains what went wrong. Check `debug.log` in the PHC-Portal folder for more detail.
...
- [Discord](https://discord.gg/RNNg5TS7h5) — ask for help
