# PHC-Portal — URI Protocol Reference
...
function downloadGame(appId) {
  ...
  setTimeout(() => {
    if (document.hasFocus() && Date.now() - before >= 1400) {
      if (confirm('PHC-Portal not detected. Open the download page?')) {
        window.location = 'https://github.com/PHC-Team/PHC-Portal/releases/latest';
      }
    }
    frame.remove();
  }, 1500);
